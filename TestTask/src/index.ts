...
class Match3Game {
    ...
    private setupEventListeners(): void {
        if (this.isMobile) {
            ...
            this.canvas.addEventListener('touchend', (e: TouchEvent) => this.onTouchEnd(e), { passive: false });
        } else {
    ...
    private getTouchPosition(touch: Touch): Position {
        ...
        const x: number = Math.floor((touch.clientX - rect.left) / CELL_SIZE);
        const y: number = Math.floor((touch.clientY - rect.top) / CELL_SIZE);
        return { x, y };
    }

    private onTouchStart(e: TouchEvent): void {
        e.preventDefault();
        if (this.isSwapping || this.isAnimating || e.touches.length !== 1) return;
        
        const touch = e.touches[0];
        const pos: Position = this.getTouchPosition(touch);
    ...
    private onTouchMove(e: TouchEvent): void {
        e.preventDefault();
        if (!this.isDragging || !this.selectedItem || this.isSwapping || this.isAnimating || e.touches.length !== 1) return;
        
        const touch = e.touches[0];
        const deltaX: number = touch.clientX - this.dragStartX;
        const deltaY: number = touch.clientY - this.dragStartY;
    ...
    private update(): void {
        for (let y = 0; y < GRID_SIZE; y++) {
            for (let x = 0; x < GRID_SIZE; x++) {
                const item: GameItem | null = this.grid[y] && this.grid[y][x] ? this.grid[y][x] : null;
                if (item) {
                    const isAnimating: boolean = item.update();
                    if (item.scale <= 0) {
                        this.grid[y][x] = null;
                    }
                }
...
const canvasElement: HTMLCanvasElement | null = document.getElementById('gameCanvas') as HTMLCanvasElement;
if (canvasElement) {
    try {
        const game: Match3Game = new Match3Game(canvasElement);
    } catch (error) {
